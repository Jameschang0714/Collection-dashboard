...
### **V19.3: ???????????? (Outbound Call Floor Highlight)**

*   **???? (Strategic Rationale)**: ?????????,????????????,???????????????????,????????????????
*   **???? (Technical Implementation)**:
    1.  **??????**:? `??????????.xlsx` ?? `??????` ??,?? 350 ?,?????????
    2.  **??????/?????**:`dashboard_local.py` ? `dashboard_cloud.py` ????????????????????,????????????????(????????)?
*   **???? (Final Outcome)**: ?????????,???????????????????? 350 ????,????????????,??????????????????

### **V19.4: 9 碼 KPI 檔名解析再強化 (Accurate Mapping for 9-Digit KPI Filenames)**

*   **戰略動機 (Strategic Rationale)**: 新一批 KPI CSV 採用 9 碼日期（例如 202501010 代表 2025-10-10），舊版只辨識 8 碼，導致 10 月中下旬資料無法掛載到儀表板。
*   **技術實作 (Technical Implementation)**:
    1.  **日期解析優先順序調整**：在 `merge_and_enrich_reports.py` 的 `extract_report_date_from_filename()` 內新增 9 碼 (YYYYMMMDD) 的拆解與驗證，並在成功時提前回傳。
    2.  **回退策略**：若 9 碼格式不符合，再回退到原本的 8 碼 (YYYYMMDD) 流程，確保舊檔仍相容。
*   **最終成果 (Final Outcome)**: KPI 檔案與通話日誌可完整映射，10/10~10/26 的資料缺口已消失，儀表板各視圖恢復正常顯示。
### **V19.2: KPI 檔名解析修復與熱力圖跳轉精簡 (KPI Filename Parsing Fix & Heatmap Jump Streamlining)**

*   **戰略決策 (Strategic Rationale)**: 十月起的 KPI 檔案改以 9 碼日期命名，導致在手案件數無法匯入；同時使用者反映月度熱力圖的快速跳轉需要縮短操作路徑，因此決議同步修復資料解析並精簡跳轉流程。
*   **技術實現 (Technical Implementation)**:
    1.  **新增日期解析輔助函數**：在 `merge_and_enrich_reports.py` 引入通用的 `extract_report_date_from_filename`，支援 8 碼與 9 碼日期格式，並重新建立 KPI 檔案映射表以涵蓋 2025/10 起的日報。
    2.  **調整熱力圖跳轉介面**：本地與雲端儀表板的月度熱力圖移除催員下拉，將日期選單與按鈕縮小後置於組別標題右側，並自動帶入所選組別＋日期觸發每日報告。
*   **最終成果 (Final Outcome)**: 十月在手案件數重新回填，月度熱力圖的快速跳轉僅需兩步即可完成，減少使用者誤選與操作時間。
